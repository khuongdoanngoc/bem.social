...
export default function Input({ label, error, errorMessage, className, id, ...props }: InputProps) {
    return (
        <div>
            <label
                htmlFor={id}
            ...
            <input
                ...
                {...props}
            />
            {error && <label className="text-red-500 font-normal ml-1">{errorMessage}</label>}
        </div>
    );
}
